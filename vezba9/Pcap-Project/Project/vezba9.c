...
int main()
{
    int i=0;
    int device_number;
    int sentBytes_udp,sentBytes_tcp;
	pcap_if_t* devices;
	pcap_if_t* device;
	...
	if ((sentBytes_udp = pcap_sendqueue_transmit(device_handle_out, queue_udp, 0)) < queue_udp->len)
	{
		printf("An error occurred sending the packets: %s. Only %d bytes were sent\n", pcap_geterr(device_handle_out), sentBytes_udp);
	}

	if ((sentBytes_tcp = pcap_sendqueue_transmit(device_handle_out, queue_tcp, 1)) < queue_tcp->len)
	{
		printf("An error occurred sending the packets: %s. Only %d bytes were sent\n", pcap_geterr(device_handle_out), sentBytes_tcp);
	}
...
void packet_handler(unsigned char* user, const struct pcap_pkthdr* packet_header, const unsigned char* packet_data)
{
	...
	if (ntohs(eh->type) == 0x800)	// Ipv4
	{
		...
		if(ih->next_protocol == 17) // UDP
		{
			...
			/**************************************************************/
		} else if(ih->next_protocol == 0x6) // TCP
		{
			/**************************************************************/
			// Add packet in the queue
			struct pcap_pkthdr* packet_header1 = packet_header;
			packet_header1->ts.tv_sec = sec;
			sec += 2;
			if (pcap_sendqueue_queue(/*(pcap_send_queue*)*/queue_tcp, packet_header1, packet_data) == -1)
			{
